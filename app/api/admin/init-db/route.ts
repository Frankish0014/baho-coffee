...
export async function POST() {
  try {
    ...
    await InventoryStorage.initializeDefaultInventory();
    await NewsletterStorage.initialize();

    return NextResponse.json({
      success: true,
      message: "Database tables initialized successfully! Inventory initialized with default products. Newsletter table created."
    }, { status: 200 });
