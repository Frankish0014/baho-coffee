...
export default function RoasterPortal() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const companyValidation = validateCompany(formData.companyName);
    const nameValidation = validateName(formData.name);
    const emailValidation = validateEmail(formData.email);
    const messageValidation = validateMessage(formData.message);
    
    const newErrors: typeof errors = {};
    if (!companyValidation.valid) newErrors.companyName = companyValidation.error;
              ...
              <div>
                ...
                <input
                  ...
                  onBlur={(e) => {
                    const validation = validateCompany(e.target.value);
                    if (!validation.valid) {
                      setErrors({ ...errors, companyName: validation.error });
                    }
                  }}
                  className={`w-full px-4 py-3 border rounded-lg bg-white dark:bg-gray-900 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500 ${
                    errors.companyName 
