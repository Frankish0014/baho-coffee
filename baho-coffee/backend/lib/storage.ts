...
export class Storage {
  ...
  static async saveSampleRequest(data: {
    ...
    if (this.isPostgresConfigured()) {
      try {
        await PostgresStorage.saveSampleRequest(data);
        return;
      } catch (error) {
        console.error("❌ Error saving to Postgres, falling back to file:", error);
        // Fall through to file system
    ...
    // Fallback to file system (local development)
    await this.saveToFile("sample-requests", [data], true);
  }
  ...
  static async getSampleRequests(): Promise<any[]> {
    if (this.isPostgresConfigured()) {
      try {
        return await PostgresStorage.getSampleRequests();
      } catch (error) {
        console.error("❌ Error loading from Postgres, falling back to file:", error);
        // Fall through to file system
      }
    ...
    // Fallback to file system (local development)
    return await this.loadFromFile("sample-requests");
  }

  /**
   * Save payment (with file fallback)
   */
  static async savePayment(data: {
    ...
    // Try to use PaymentStorage (PostgreSQL) if available
    try {
      const { PaymentStorage } = await import("./db/payments");
      if (this.isPostgresConfigured()) {
        try {
          await PaymentStorage.savePayment(data as any);
          return;
        } catch (error) {
          console.error("❌ Error saving payment to Postgres, falling back to file:", error);
          // Fall through to file system
        }
      }
    } catch (error) {
      console.error("❌ Error importing PaymentStorage, falling back to file:", error);
      // Fall through to file system
    }

    // Fallback to file system (local development)
    await this.saveToFile("payments", [data], true);
  }

  /**
   * Get all payments (with file fallback)
   */
  static async getPayments(): Promise<any[]> {
    ...
    try {
      ...
      if (this.isPostgresConfigured()) {
        try {
          return await PaymentStorage.getAllPayments(1000);
        } catch (error) {
          console.error("❌ Error loading payments from Postgres, falling back to file:", error);
          // Fall through to file system
        }
      }
    } catch (error) {
    ...
    // Fallback to file system (local development)
    return await this.loadFromFile("payments");
  }
  ...
  static async append(key: string, newItem: any): Promise<void> {
    ...
    } else if (key === "sample-requests") {
      await this.saveSampleRequest(newItem);
    } else {
